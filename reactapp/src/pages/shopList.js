...
class Customers extends Component {
    ...
    componentDidMount() {
      ...
      ajaxWrapper('GET','/api/home/order/?related=food_item,food_item__shopping_list&event=' + this.props.event_id, {}, this.eventCallback)
    }
    ...
    eventCallback(result) {
      ...
      for (var index in result) {
        var shoppinglist = result[index]['order']['food_item']['shopping_list']
    ...
    render() {
      ...
      for (var index in this.state.orders) {
        ...
        for (var index2 in this.state.orders[index][2]) {
          var item = this.state.orders[index][2][index2]['shoppinglistitem']
          todolist.push(<Checkbox name={'None'} label={'________ ' + item['task']} value={0} />)
        }
        var renderItem = <div className="col-md-6" style={{'marginBottom':'25px'}}><div style={{'color':'#27932f'}}><Header size={5} text={this.state.orders[index][0]}  /></div> {todolist} <br /></div>;
      ...
      var content =
        ...
        <div style={{'marginTop':'25px'}}>
          ...
          <Header css={{'padding':'50px','color':'#27932f', 'text-align':'center'}} size={1} text={'Shopping List for ' + this.state.eventInfo.name} />
          {eventInfo}
          <div style={{'marginTop':'35px','marginBottom':'50px'}}>
          <Header size={2} text={'Shop List'} />
          </div>
          <div className="row">
          {renderList}
          </div>
