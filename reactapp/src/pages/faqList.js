...
class OrderableList extends Component {
  ...
  componentDidMount() {
    ajaxWrapper('GET','/api/home/faq/?order_by=order',{}, this.questionCallback);
  }
  ...
  questionCallback(result) {
    ...
    for (var index in result) {
      var question = result[index]['faq']
      var card = <Card name={question['question']} link={'/editFAQ/' + question.id + '/'} button_type={'primary'} button={'Edit Question'} />
  ...
  updatePreviewData(questions, questions_archived) {
    ...
    if (questions) {
      for (var index in questions) {
        ajaxWrapper('POST','/api/home/faq/' + questions[index]['id'] + '/', {'preview_archived':false, preview_order:index}, console.log)
      }
    }
    if (questions_archived) {
      for (var index in questions_archived) {
        ajaxWrapper('POST','/api/home/faq/' + questions_archived[index]['id'] + '/', {'preview_archived':true, preview_order:index}, console.log)
      }
    }
  }
  ...
  updateData() {
    ...
    for (var index in questions) {
      ajaxWrapper('POST','/api/home/faq/' + questions[index]['id'] + '/', {'preview_archived':false, preview_order:index, 'archived':false, order:index}, console.log)
    }
    for (var index in questions_archived) {
      ajaxWrapper('POST','/api/home/faq/' + questions_archived[index]['id'] + '/', {'preview_archived':true, preview_order:index, archived:true, order:index}, console.log)
    }
  }
    /**
    ...
    render() {
        return (
          <div className="container">
            <Button type={'info'} href={'/editFAQ/'} text={'Add New Question'} />
            <Button type={'success'} clickHandler={this.updateData} text={'Save Order'} />
            <Header size={3} text={'FAQ Questions'} />
